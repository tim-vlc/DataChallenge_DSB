...
os.system("pip install holidays lockdowndates haversine meteostat xgboost pyarrow fastparquet")
...
X_train, y_train = _read_data('/kaggle/input/mdsb-2023/train.parquet')
X_final = pd.read_parquet('/kaggle/input/mdsb-2023/final_test.parquet')
...
def _closest_transport(X): 
    ...
    idf_stations = pd.read_csv('/kaggle/input/idf-dataset/Stations_IDF.csv', delimiter=';', header=None, names=column_names)
...
def _add_weather_data(X):
    ...
    return X.drop(columns=["date", "counter_name", "site_id", "site_name", 
                           ...
                           "longitude", "latitude", "date_ws", "year"])
...
def get_estimator():
    ...
    categorical_cols = ["counter_id", "closest_metro_distance", "holidays", "france_stay_at_home"]
