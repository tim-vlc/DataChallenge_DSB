...
vacances_scolaires = pd.read_csv('../../data/vacances-scolaires.csv')
vacances_scolaires['date'] = pd.to_datetime(vacances_scolaires['date'])
vacances_scolaires = vacances_scolaires[vacances_scolaires['date'].dt.year.isin([2020, 2021])].copy()
vacances_scolaires = vacances_scolaires[vacances_scolaires['vacances_zone_c']==True]
vacances_scolaires = vacances_scolaires[['date']]

vacances_scolaires.to_csv('test.csv')

def is_datetime_in_dataframe(datetime_object, dataframe):
    date_part = datetime_object.date()

    return date_part in dataframe['date'].dt.date.values
...
def _encode_dates(X):
    ...
    # Add the column corresponding to school holidays
    # Paris is in zone 'C' of the school holidays in France
    X.loc[:, 'school_holidays'] = X['date_ws'].isin(vacances_scolaires['date'].dt.date).astype(int)
...
def get_estimator():
    ...
    categorical_cols = ["counter_id", "holidays", "france_stay_at_home", "year", "school_holidays"]
    numerical_cols = ['temp', 'rhum', 'wspd', 'prcp', "closest_metro_distance"]
    ...
    processing_pipe = make_pipeline(date_encoder, transport_encoder, weather_encoder, data_processor)

    return pipe, processing_pipe

pipe, processing_pipe = get_estimator()
pipe.fit(X_train, y_train)

X_trans = processing_pipe.fit_transform(X_train)
corr_mat = pd.DataFrame(X_trans).corr()
