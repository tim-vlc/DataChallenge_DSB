...
def get_estimator():
    ...
    regressor = CatBoostRegressor(iterations=2000, depth=11, eval_metric='RMSE')
...
for i, (counter, dates) in enumerate(sanity_dict.items()):
    ...
    mask = (final_pred['counter_name'] == counter) & (pd.to_datetime(final_pred['date']) > threshold_date) & (pd.to_datetime(final_pred['date']) < top_date)
    final_pred.loc[mask, 'log_bike_count'] = 0

y_pred = final_pred['log_bike_count'].to_numpy()
